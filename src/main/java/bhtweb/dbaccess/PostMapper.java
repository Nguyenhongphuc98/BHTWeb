...
public class PostMapper extends DBMapper {
	...
	private static final String insertPostStr = "INSERT INTO POST VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)";
	private static final String updatePostStr = "UPDATE POST SET PostTitle = %s, PostContentURL = %s, PostSubmitDtm = %s, PostPublishDtm = %s, PostReadTime = %s, NumVote = %s, NumView = %s, PostSoftDeleted = %s, PostHidden = %s, PostApproved = %s, PosterUserID = %s, PostCategoryID = %s WHERE PostID = %s";
	private static final String fetchPostStr = "SELECT * FROM POST LIMIT %s,%s ORDER BY postPublishDtm DESC";
	private static final String searchPostStr = "SELECT * FROM POST WHERE PostTitle LIKE %?% LIMIT %s,%s ORDER BY postPublishDtm DESC";
	...
	public PostDTO insertPost (PostDTO postDTO) {
		...
		//Câu lệnh cho phép insert một DTO.
		String string = String.format(format, args)
		
		try {
			Statement insertPostSt = getConnection().createStatement();
			Statement insertPostPst = null;
			insertPost
